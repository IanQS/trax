...
class ActorCriticTrainer(rl_training.PolicyTrainer):
  ...
  def __init__(self, task,
    ...
    Args:
     ...
     replay_buffer_size: how many last epochs to take into the replay buffer;
  ...
  def _replay_epochs(self):
    if self.on_policy:
      assert self._replay_buffer_size == 1, (
          'Non-unit replay buffer size only makes sense for off-policy '
          'algorithms.'
      )
    return [-(ep + 1) for ep in range(self._replay_buffer_size)]
